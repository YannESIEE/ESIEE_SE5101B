...
static void updateSignalLevels(int sender, double level);
static void send_message(NWmsg *m, double *nextHit);
...
RTnetwork *nwsys;
//FIX
double (*pathloss) (double transmitPower, int node1, double x1, double y1, int node2, double x2, double y2);
char *codeName;
...
static int snr_ok(NWmsg *m){
  ...
  if ( ber>0 ){
    ...
    //printf("bit error rate is below %f with probability %f\n", nwsys->error_threshold, success_probability);

    if ( (rand()/(double)RAND_MAX) > success_probability ){
      return 0;
    }
  }
  return 1;
 ...
 */
void broadcast(NWmsg *m, double *nextHit) {
  int j;
  NWmsg *m2;
  mxArray* data = NULL;
  ...
  updateSignalLevels(m->sender, nwsys->nwnodes[m->sender]->transmitPower);
  ...
  for (j=0; j<nwsys->nbrOfNodes; j++) {
	...
	mexMakeArrayPersistent(m2->dataMATLAB);
      ...
      //printf("broadcasting to node %d\n", m2->receiver+1);
      send_message(m2, nextHit);
    }
  }
...
static void send_message(NWmsg *m, double *nextHit){
  ...
  //printf("moving message from node %d to node %d from preprocQ to inputQ at %f\n",m->sender+1, m->receiver+1, nwsys->time);
  ...
  if ( nwsys->nwnodes[m->receiver]->state == W_SENDING ) {
    m->collided = 1;
    //printf("can not send and receive at the same time (node %d), message from %d\n", m->receiver+1, m->sender+1);
  } 
  ...
  for (j=0; j<nwsys->nbrOfNodes; j++) {
    if ( j!=m->sender && nwsys->nwnodes[m->receiver]->signallevels[j]>=nwsys->nwnodes[m->receiver]->signallevels[m->sender] ){
      m->collided = 1;
      //printf("Collision with another message in receiver (node %d) at time %f\n", m->receiver+1, nwsys->time);
      //printf("\t marking messages in %d from (node %d) as collided\n",j+1, m->sender+1);
      break;
    }
  }
  ...
  for (j=0; j<nwsys->nbrOfNodes; j++) {
	...
	//printf("marking messages in %d from (node %d) as collided at time %f\n",j+1, m2->sender+1, nwsys->time);
	m2->collided = 1;
      }
 ...
 */
static void resend_message(NWmsg *m, double *waituntil){
  NWmsg *m2;
  ...
  if ( m->type!=BROADCAST && 
    ...
    //printf("message to (node %d) from (node %d) put in preProcQ again\n", m->receiver+1, m->sender+1);
    ...
    //printf("node %d chooses a backoff of %f slots at time %f\n",
    ...
    *waituntil = nwsys->time + nwsys->nwnodes[m->sender]->backoff; 
    ...
    // Drop the message
    //printf("The message from (node %d) to (node %d) has been dropped at time %f\n",m->sender+1, m->receiver+1, nwsys->time);
 ...
 */
static double c_pathloss(double transmitPower, int node1, double x1, double y1, int node2, double x2, double y2){

  double distance = sqrt(pow(x1 - x2, 2.0) + pow(y1 - y2, 2.0));
  //printf("distance is %f m\n",distance);

  return transmitPower/pow(distance+1, nwsys->pathloss);
}
 ...
 * transmitPower, amount of Watts that the sending node is sending with
 */
static double matlab_pathloss(double transmitPower, int node1, double x1, double y1, int node2, double x2, double y2){
  double retval;
  mxArray *lhs[1];
  mxArray *rhs[8];
  ...
  *mxGetPr(rhs[7]) = nwsys->time;
  ...
  if (mexCallMATLAB(1, lhs, 8, rhs, codeName) != 0) {
    printf("Error while trying to call the user specified pathloss function\n");
    return 0.0;
  }
 ...
 */
static void updateSignalLevels(int sender, double transmitPower){
  int i;
  double oldvalue;
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
    ...
    if (transmitPower == 0){
      ...
      nwsys->nwnodes[i]->signallevels[sender] = (*pathloss)(transmitPower, sender, x_sender, y_sender, i, nwsys->nwnodes[i]->xCoordinate, nwsys->nwnodes[i]->yCoordinate);
    }
...
static int backoff_802_11(int nbr){
  ...
  //printf("\t using collision window of size %d at time %f\n", collisionWindow, nwsys->time);
  return urand(0, collisionWindow);
}



static void MAC_802_11(double *nextHit){
  NWmsg *m;
  int i;
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
    //    printf("\t Checking preProcQ of node %d at %f\n",i+1, nwsys->time);
	...
	//printf("waited the preproc (ACKTimeout) time (node %d) at %f\n",
	//m->sender+1, nwsys->time);	
	// Set the waiting flag if we are not already sending
	if (nwsys->nwnodes[m->sender]->state != W_SENDING){
	  //printf("Already sending in (node %d) at time %f\n", 
	  //m->sender+1, nwsys->time););
	  nwsys->nwnodes[m->sender]->state = W_WAITING;
	}

	// no signal during the last DIFS seconds?
	if ( nwsys->nwnodes[i]->lastused + DIFS_802_11 <= nwsys->time + EPS) {
	  //printf("the net is free in node %d\n", i+1);
	  ...
	  if ( nwsys->nwnodes[i]->backoff > 0 ){
	    //printf("counting down the backoff (node %d)\n",i+1);
	  ...
	  if ( nwsys->nwnodes[i]->backoff < EPS ){
	    if (m->receiver == -1){ // -1 <==> broadcast
	      broadcast(m, nextHit);
	    } else{
	      send_message(m, nextHit);
	    }
...
static void MAC_802_15_4(double *nextHit){
  NWmsg *m = NULL;
  int i;
...
#define IFS SIFS
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
	...
	*nextHit = m->waituntil;
    ...
    if ( nwsys->nwnodes[i]->backoff - nwsys->time > EPS ){
      // When are we finished waiting?
	...
	*nextHit = nwsys->nwnodes[i]->backoff;
    ...
    if ( m->nbrOfBackoffs == -1 ){
      m->nbrOfBackoffs = 0;
      nwsys->nwnodes[i]->backoff = 
	urand(0, 1<<macMinBE)*aUnitBackoffPeriod*symbolTime + nwsys->time;
      //printf("Initial backoff: %f at time: %f in node: %d\n", nwsys->nwnodes[i]->backoff, nwsys->time, i+1);
      // update nextHit
      if ( nwsys->nwnodes[i]->backoff < *nextHit) {
	*nextHit = nwsys->nwnodes[i]->backoff;
      }
    ...
    // Perform CCA
    if ( nwsys->nwnodes[i]->lastused + IFS*symbolTime - nwsys->time <= EPS) {
    ...
    if ( m->nbrOfBackoffs > macMaxCSMABackoffs ){
      ...
      //printf("* Dropping message from node:%d to node:%d, at time %f (maximum CSMA backoffs reached)\n", m->sender+1, m->receiver+1, nwsys->time);
#ifndef KERNEL_C
      mxDestroyArray(m->dataMATLAB);
#endif
    ...
    nwsys->nwnodes[i]->backoff = 
      urand(0, 1<<temp) * aUnitBackoffPeriod * symbolTime + nwsys->time;
    //printf("\tBackoff: %f at time: %f in node: %d\n", nwsys->nwnodes[i]->backoff, nwsys->time, i+1);
    if ( nwsys->nwnodes[i]->backoff < *nextHit) {
      *nextHit = nwsys->nwnodes[i]->backoff;
    }
  } // for (i=0; i<nwsys->nbrOfNodes; i++) {
...
static double runNetwork() {
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
    ...
    while (m != NULL) {
      ...
      //printf("Counting down transmission time of node %d\n",m->sender+1);
      //printf("\t remaining %f\n",m->remaining);
      // frame finished?
      if (m->remaining < EPS) {
	...
	if (m->collided || nwsys->nwnodes[i]->signallevels[m->sender] < 
	  ...
	  //printf("want to resend message from node %d to node %d at time %f\n",m->sender+1, m->receiver+1, nwsys->time);
	  resend_message(m, &waituntil);
	  ...
	  if ( snr_ok(m) ){
	    ...
	    //printf("* message put in outputQ (node %d) from (node %d) at time %f\n", m->receiver+1, m->sender+1, nwsys->time);
	    ...
	    resend_message(m, &waituntil);
	    //printf("packet lost due to to large BER in node %d from node %d at time\n", m->receiver, m->sender+1, nwsys->time);
	  }
	...
	if (nwsys->time + remxtime(m) < nextHit) {
	  nextHit = nwsys->time + remxtime(m);
	  //printf("Transmission from (node %d) to (node %d) is finished at %f\n",m->sender,m->receiver,nwsys->time + remxtime(m));
	}	  
      }
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
	...
	//printf("*** moving message from outputQ (node %d) to postprocQ at %f\n", i+1, nwsys->time);
  ...
  for (i=0; i<nwsys->nbrOfNodes; i++) {
    if (nwsys->nwnodes[i]->state == W_SENDING) {
      ...
      //Something is wrong
      //printf("Error creating schedule file:%s line:%d\n",__FILE__,__LINE__);
    }
  }
...
#include "simstruc.h"
  static void mdlInitializeSizes(SimStruct *S)
  {
    ...
    if (!printed2) {
      printed2 = 1;
      printf("Wireless network data:\n");
      printf("Transmit power is:\t%.2f dbm \t<==> %.2f mW\n", 
	     temp1, transmitPowerWatt*1000);
      printf("Receiver threshold is:\t%.2f dbm \t<==> %.2e mW\n",
	     temp2, nwsys->receiverThreshold*1000);
      printf("Maximum signal reach is calculated to:\t%.2f m\n", 
	     pow(transmitPowerWatt/nwsys->receiverThreshold,
		 1/nwsys->pathloss)-1);
      printf("\n");
    }
    
    // 9 - Special pathloss function checkbox
    arg = ssGetSFcnParam(S, 8);
    if (mxIsChar(arg)) {
      char buf[100];
      mxGetString(arg, buf, 100);
      if (strcmp(buf, "on")==0){
#ifdef KERNEL_C
	ssSetErrorStatus(S, "The special pathloss function can NOT be used in C++ mode");
	return;
#else
	// 10 - Special pathloss function if the checkbox is checked
	arg = ssGetSFcnParam(S, 9);
	if (mxIsChar(arg)) {
	  char buf[100];
	  mxGetString(arg, buf, 100);
	  codeName = new char[strlen(buf)+1];
	  strcpy(codeName, buf);
	  pathloss = &matlab_pathloss;
	} else {
    ...
    //printf("Error coding threshold=%f %f\n",nwsys->error_threshold, *mxGetPr(arg));
...
#define MDL_INITIALIZE_CONDITIONS
  ...
  static void mdlOutputs(SimStruct *S, int_T tid)
  {
    ...
    // No significant speedup using memcpy instead of looping
    //memcpy(output_0, nwsys->outputs, nwsys->nbrOfNodes*sizeof(double));
    //memcpy(output_1, nwsys->sendschedule, nwsys->nbrOfNodes*sizeof(double));
    //memcpy(output_2, nwsys->energyconsumption, nwsys->nbrOfNodes*sizeof(double));
    for (i=0; i<nwsys->nbrOfNodes; i++) {
...
#define MDL_ZERO_CROSSINGS
  ...
  static void mdlTerminate(SimStruct *S)
  {
    ...
    //sprintf(nwsysbuf, "network%d", nwsys->networkNbr);
    //char *nametemp[]= {"nbrOfTransmissions", "nbrOfSuccessfullReceptions"};
    //mxArray *temp = mxCreateStructMatrix(1, 1, 2, (const char**)&nametemp);
    //mxSetField(temp, 0, "nbrOfTransmissions", nwsys->nbrOfTransmissions);
    //mxSetField(temp, 0, "nbrOfSuccessfullReceptions",mxDuplicateArray(nwsys->nbrOfTransmissions));
	       //nwsys->nbrOfSuccessfullReceptions);
	       
    //mexPutVariable("global", nwsysbuf, temp);
